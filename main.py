...
async def contaminacion(ctx, type = "GENERAL"):
    if type.upper() == "GENERAL":
        await ctx.send(general)
    if type.upper() == "AIR":
        await ctx.send(Air_Pollution_en)
    if type.upper() == "WATER":
        await ctx.send(Water_Contamination_en)
    if type.upper() == "SOIL":
        await ctx.send(Soil_Contamination_en)
    if type.upper() == "SOUND":
        await ctx.send(Sound_Contamination_en)
    if type.upper() == "LIGHT":
        await ctx.send(Light_Contamination_en)
    if type.upper() == "THERMAL":
        await ctx.send(Thermal_Pollution_en)
    if type.upper() == "RADIOACTIVE":
        await ctx.send(Radioactive_Contamination_en)
    if type.upper() == "PLASTIC":
        await ctx.send(Plastic_Pollution_en)
    if type.upper() == "WASTE":
        await ctx.send(Hazardous_Waste_Pollution_en)
    if type.upper() == "BIOLOGICAL":
        await ctx.send(Biological_Contamination_en)
    if type.upper() == "GROUNDWATER":
        await ctx.send(Groundwater_Contamination_en)
    if type.upper() == "ELECTROMAGNETIC":
        await ctx.send(Electromagnetic_Pollution_en)
    if type.upper() == "CHEMICAL":
        await ctx.send(Chemical_Spills)
    if type.upper() == "ALLERGENS":
        await ctx.send(Airborne_Allergens_en)
    if type.upper() == "ACID_RAIN":
        await ctx.send(Acid_Rain_en)
